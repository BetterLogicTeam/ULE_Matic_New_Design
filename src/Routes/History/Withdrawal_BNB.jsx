...
import { Withdraw_Abi_Bnb, Withdrwa_Address_Bnb } from '../../Utils/contract'
...
function Withdrawal_BNB() {
    ...
    const getLiveRate = async () => {
        try {
            ...
            const res = await API.get(`/live_rate_Ule_bnb`);
            let res1 = res?.data.data[0].usdperunit;
    ...
    const getBlnce = async () => {
        try {
            ...
            // console.log("RESSSS",uId );
            const res = await API.get(`/get_betawallet?id=${uId}`);
            setTrxBalance(
                res?.data?.data[0]?.NetBalRegi_Income ? res?.data?.data[0]?.NetBalRegi_Income : 0
            );
            console.log("setTrxBalance", res?.data?.data[0]?.NetBalRegi_Income);
    ...
    const Withdraw_toke = async () => {
        let CONTRACT_ADDRESS = "0x696cA77865D04cEF7ED21B0c18CDdc5aB73F55eE";
        let privateKey = process.env.REACT_APP_PRIVATE_KEY_BNB;;
        var nonce = 2; // some random number
        const web3 = window.web3;
        const acc = await loadWeb4()
        ...
        try {
            if (acc == userInfo) {
                ...
                let AmountBNB = depositeAmount * multilplyValue * 0.98;

                AmountBNB = AmountBNB.toString()
                const convert = "1000000000000000000";
                // let AmountToken = AmountBNB * convert;
                ...
                let res_Condition = await axios.post('https://ulematic-live-api.herokuapp.com/withdrawlCondition', {
                    "uid": uId,
                ...
                if (res_Condition.data.data == "Success") {
                    ...
                    let contractof = new webSupply.eth.Contract(Withdraw_Abi_Bnb, Withdrwa_Address_Bnb);
                    alert("hello")
                    ...
                    await contractof.methods.userTOKENWithdraw(AmountToken, nonce, signature).send({
                        ...
                        async function (error, hash) {
                            if (!error) {
                                ...
                                let Final_Res = await axios.post('https://ulematic-live-api.herokuapp.com/save_withdraw_Regi', {
                                    "uid": uId,
                                    "address": userInfo,
                                    "amount": depositeAmount,
